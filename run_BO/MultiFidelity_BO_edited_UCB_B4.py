...
def EI_hf(model, X, acquired_set):
    ...
    beta = 4  # exploration parameter - adjust as needed
...
bot_ucb = UpperConfidenceBound(bogus_model, beta=4)
...
def save_run_results(acquired_set, run, ablation_study_flag):
    ...
    import os  
    import pickle
    os.makedirs('../search_results/mfbo/Run5_10Dec_UCB_B4', exist_ok=True)

    pickle_filename = '../search_results/mfbo/Run5_10Dec_UCB_B4/mfbo_results_run_{}'.format(run)
    if ablation_study_flag:
