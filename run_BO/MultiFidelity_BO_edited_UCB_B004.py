...
def EI_hf(model, X, acquired_set):
    ...
    beta = 0.04  # exploration parameter - adjust as needed
...
bot_ucb = UpperConfidenceBound(bogus_model, beta=0.04)
...
def save_run_results(acquired_set, run, ablation_study_flag):
    ...
    import os  
    import pickle
    os.makedirs('../search_results/mfbo/Run5_10Dec_UCB_B004', exist_ok=True)

    pickle_filename = '../search_results/mfbo/Run5_10Dec_UCB_B004/mfbo_results_run_{}'.format(run)
    if ablation_study_flag:
