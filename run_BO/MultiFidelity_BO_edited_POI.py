...
def EI_hf(model, X, acquired_set):
    ...
    poi = norm.cdf(z)
    return np.maximum(poi, np.zeros(X.size()[0]))
...
# use BO Torch's accquisition functions
bot_ei = ProbabilityOfImprovement(bogus_model, best_f=get_y_max(bogus_acquired_set, 1))
...
def save_run_results(acquired_set, run, ablation_study_flag):
    ...
    pickle_filename = '../search_results/mfbo/Run3_10Dec_Saloni_POI/mfbo_results_run_{}'.format(run)
    if ablation_study_flag:
